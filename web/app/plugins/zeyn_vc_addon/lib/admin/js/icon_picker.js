    ...
    $.fn.iconPicker = function( options ) {
        ...
        return this.each( function() {
            ...
            if(!settings.buttonOnly && $(this).data("iconPicker")==undefined ){
				...
				var $value=$parent.find('.wpb_vc_param_value.iconlists_field').val(),
				$activeicon='icon-picture';
				if($value!=''){
					$activeicon=$value;
				}


            	$button=$("<span class=\"input-group-addon pointer\"><i class=\""+$activeicon+"\"></i></span>");
            	$this.after($button);
	        ...
	        function showList($element,arrLis){
	        	...
	        	$(".iconlist li a",$popup).click(function(e){
	        		...
	        		if (typeof (settings.onUpdate) === 'function') {
						...
						$parent.find('.wpb_vc_param_value.iconlists_field').val('icon-'+title);
						$parent.find('.input-group-addon').html('<i class="icon-'+title+'"></i>');
	        		}
