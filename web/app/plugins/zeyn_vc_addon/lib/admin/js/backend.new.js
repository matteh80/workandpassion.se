jQuery(document).ready(function ($) {
    ...
    if($('.optin_button_preview').length){
    	$('.optin_button_preview').each(function(){
  		...
  		var $preview=$(this),$parent=$(this).closest('.vc_panel-tabs');
    		...
    		$('.name_label',$parent).keyup(function(){
    			$('input[name=dt_name]').val($(this).val());
    		}).trigger('keyup');

    		$('.email_label',$parent).keyup(function(){
    			$('input[name=dt_email]').val($(this).val());
    		}).trigger('keyup');
	    	...
	    	$('input[name=background_color]',$parent).on('change',function(){
	    		optinform.css('background',$(this).val());

	    	})
	    	.wpColorPicker({
	    		change:function(event,ui){
	    			optinform.css('background',ui.color.toString());
		        }
	    	}).trigger('change');


	    	$('input[name=background_color]',$parent).closest('.wp-picker-container').find('.wp-picker-clear').on('click',function(){
	    		optinform.css('background-color','inherit');
	    	});

	    	$('input[name=border_color]',$parent).on('change',function(){
	    		optinform.css('border-color',$(this).val());
	    	}).wpColorPicker({
	    		change:function(event,ui){
	    			optinform.css('border-color',ui.color.toString());
		        }
	    	}).trigger('change');

	    	$('input[name=border_color]',$parent).closest('.wp-picker-container').find('.wp-picker-clear').on('click',function(){
	    		optinform.css('border-color','inherit');
	    	});
    	});
    }
});
